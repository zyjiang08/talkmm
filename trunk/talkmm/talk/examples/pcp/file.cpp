...
class FileShareClient : public sigslot::has_slots<>, public talk_base::MessageHandler {
 public:
  FileShareClient(gloox::Client *xmppclient, const buzz::Jid &send_to, const cricket::FileShareManifest *manifest, std::string root_dir) :
...
//      std::cout << "Logged out." << std::endl;
 ...
 private:
  ...
  void OnSignon() {
    std::string client_unique = xmpp_client_->jid().full();
    ...
    */
    ...
    file_share_session_client_.reset(new cricket::FileShareSessionClient(session_manager_.get(), buzz::Jid(xmpp_client_->jid().full()), "pcp"));
                    ...
                    MessageEventHandler, MessageHandler, ChatStateHandler
{
  public:
    ...
    void start()
    {
      ...
      JID jid( "cycloneblog@gmail.com" );
      j = new Client( jid, "cycloneblog123" );
