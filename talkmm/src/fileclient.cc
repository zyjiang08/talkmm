...
FileShareClient::FileShareClient(buzz::XmppClient * xmppclient,
				 ...
				 std::
				 string root_dir):xmpp_client_(xmppclient),
root_dir_(root_dir), send_to_jid_(send_to),
waiting_for_file_(send_to == buzz::JID_EMPTY), manifest_(manifest)
{
...
void FileShareClient::
SendFile( /*const std::string& to, *//*const std::string& file */ )
{
...
std::string FileShareClient::filesize_to_string(unsigned int size)
{
	...
	ret << std::setprecision(1) << std::setiosflags(std::ios::
							fixed) <<
	    size_display << " " << format;
	return ret.str();
}
...
void FileShareClient::OnSessionState(cricket::FileShareState state)
{
	...
	case cricket::FS_OFFER:
		...
		if (manifest_ != NULL && manifest_->size() == 1)
			manifest_description << session_->manifest()->
			    item(0).name;
		else if (session_->manifest()->GetFileCount()
			 && session_->manifest()->GetFolderCount())
			manifest_description << session_->manifest()->
			    GetFileCount() << " files and " << session_->
			    manifest()->GetFolderCount() << " directories";
		else if (session_->manifest()->GetFileCount() > 0)
			manifest_description << session_->manifest()->
			    GetFileCount() << " files";
		else
			manifest_description << session_->manifest()->
			    GetFolderCount() << " directories";
		...
		if (session_->is_sender()) {
			std::cout << "Offering " << manifest_description.
			    str() << " to " << send_to_jid_.
			    Str() << std::endl;
		} else {	/* if (waiting_for_file_) */

			std::cout << "Receiving " << manifest_description.
			    str() << " from " << session_->jid().
			    Str() << std::endl;
			...
			SignalFileReceived(session_->jid().Str(),
...
//      session_->Accept();
	...
	case cricket::FS_TRANSFER:
		this->SignalFileTransferStatue(type, "started", session_->jid().Str());
	...
	case cricket::FS_COMPLETE:
		...
		this->SignalFileTransferStatue(type, "completed", session_->jid().Str());
...
//      this->manifest_ = NULL;
	...
	case cricket::FS_LOCAL_CANCEL:
		...
		this->SignalFileTransferStatue(type, "local-canceled",session_->jid().Str()); 
		break;
	case cricket::FS_REMOTE_CANCEL:
		console_->Print("filetransferfailed###\n");
		waiting_for_file_ = true;
		this->SignalFileTransferStatue(type, "canceled",session_->jid().Str()); 
	...
	case cricket::FS_FAILURE:
		console_->Print("filetransferfailed###\n");
		waiting_for_file_ = true;
		this->SignalFileTransferStatue(type, "failed", session_->jid().Str());
		break;
	}
}
		...
		for (unsigned int l = 0; l < width; l++) {
			if (l < percent * progressbar_width)
				std::cout.put('#');
				 ...
				 && l <
				 progressbar_width + 1 + strlen(filename))
				std::cout.
				    put(filename
					[l - (progressbar_width + 1)]);
			else
				std::cout.put(' ');
		}
		std::cout.flush();
...
void FileShareClient::OnResampleImage(std::string path, int width,
				      int height,
...
void FileShareClient::OnSignon(cricket::HttpPortAllocator * port_allocator, cricket::SessionManager * session_manager, buzz::JingleInfoTask * jingle_info_task,	/*pas utile */
...
*/
	file_share_session_client_.
	    reset(new cricket::
		  FileShareSessionClient(session_manager_.get(),
					 xmpp_client_->jid(), "pcp"));
	file_share_session_client_->SignalFileShareSessionCreate.
	    connect(this, &FileShareClient::OnFileShareSessionCreate);
